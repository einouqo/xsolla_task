...
- #### Удаление компании
    ...
    - Параметры:
        - key - строка (ключь, ктороы дается при регистрации компании)
- #### Регистрация
    ...
    - Параметры:
        ...
        - phone - строка, содержащая только цифры (органичена сверху 11 знаками)
...
- #### Изменение персональных данных
    - PUT-запрос `192.168.100.123:8080/settings/change`
        ...
        - после успешного изменения данных пользователь должен перелогинится
...
- #### Дать доступ к складу
    - POST-зпрос `192.168.100.123:8080/access/give`
        - Только для «Admin»
    - Параметры
        - userID - id пользователей типа «Employee», челочисленное значение
...
- #### Формирование поставки
    ...
    - Параметры
        ...
        \* товары резервивруются до тех пор, пока поставка не отправлена, либо не отменена
        - size - строка
        - quantity - положительное, целочисленное значение
...
- #### Отправить поставку
    - DELETE-запрос `192.168.100.123:8080/warehouses/items/transfers/send`
        - Только для «Employee»
    - Параметры
        - warehouseToID - id склада на который отпраляем поставку
----
